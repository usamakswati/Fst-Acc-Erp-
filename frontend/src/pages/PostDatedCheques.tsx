...
export default function PostDatedCheques({ currency }: { currency: string }) {
  ...
  const handleClear = async (id: string) => {
    if (!confirm('Clear this cheque? This will deposit the funds into the bank account and post to general ledger.')) return;
    try {
  ...
  const handleBounce = async (id: string) => {
    if (!confirm('Mark this cheque as BOUNCED?')) return;
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        >
          <Plus size={18} /> Record Post Dated Cheque
        </button>
      </div>
                  ...
                  filteredCheques.map((cheque) => (
                    <tr key={cheque.id} className="hover:bg-brand-900/10 transition-colors">
                      ...
                      <td className="p-4">
                        ...
                        <div className="text-[10px] text-slate-500">Received: {new Date(cheque.dateReceived).toLocaleDateString()}</div>
                      </td>
          ...
          <div className="w-full max-w-xl bg-brand-950 border border-brand-800/40 rounded-xl p-6 shadow-glass space-y-4">
            <div className="flex justify-between items-center border-b border-brand-800/30 pb-3">
              <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
                <CreditCard className="text-indigo-400" /> Record Post Dated Cheque Received
              </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-sm">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-slate-400 uppercase">Cheque Date (Future Clearing) *</label>
                  <input 
                    type="date" 
                    value={chequeDate} 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-slate-400 uppercase">Deposit Bank Account *</label>
                  <select 
                    value={accountId} 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-slate-400 uppercase">Issuing Bank Name *</label>
                  <input 
                    type="text" 
                    value={bankName} 
                    onChange={(e) => setBankName(e.target.value)} 
                    placeholder="e.g. Standard Chartered, Habib Bank"
                    className="w-full rounded-lg bg-brand-900/20 border border-brand-800/40 text-slate-250 p-2.5"
                    required 
                  />
                </div>
