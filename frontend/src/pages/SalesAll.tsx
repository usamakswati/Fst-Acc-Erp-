...
interface SalesTransaction {
  id: string;
  date: string;
  type: string; // INVOICE, POS, RECEIPT, PDC
  reference: string;
...
export default function SalesAll({ currency }: { currency: string }) {
  ...
  async function loadTransactions() {
    try {
      ...
      // Fetch Invoices (both regular and POS)
      const invoices = await api.getInvoices();
      const regularInvoices = invoices.filter((i: any) => !i.isPos);
      const posInvoices = invoices.filter((i: any) => i.isPos);
      ...
      // Compile stats
      const invSum = regularInvoices
        .filter((i: any) => i.status === 'APPROVED' || i.status === 'PAID')
        .reduce((sum: number, i: any) => sum + i.grandTotal, 0);
      const posSum = posInvoices
        .reduce((sum: number, i: any) => sum + i.grandTotal, 0);
      const recSum = receipts
        .reduce((sum: number, r: any) => sum + r.amount, 0);
      const pdcSum = cheques
        .filter((c: any) => c.status === 'PENDING')
        .reduce((sum: number, c: any) => sum + c.amount, 0);

      setTotalInvoices(invSum);
      setTotalPOS(posSum);
      setTotalReceipts(recSum);
      setTotalPDCs(pdcSum);

      // Map to standard transaction structure
      const txs: SalesTransaction[] = [];

      regularInvoices.forEach((i: any) => {
        txs.push({
          id: i.id,
          date: i.date,
          type: 'Standard Invoice',
          reference: i.invoiceNumber,
          name: i.contact?.name || 'Unknown',
          amount: i.grandTotal,
          status: i.status
        });
      });

      posInvoices.forEach((i: any) => {
        txs.push({
          id: i.id,
          date: i.date,
          type: 'POS Cash Sale',
          reference: i.invoiceNumber,
          name: i.contact?.name || 'Walk-in Customer',
          amount: i.grandTotal,
          status: i.status
        });
      });
      ...
      cheques.forEach((c: any) => {
        txs.push({
          ...
          name: c.contact?.name || 'Habib Bank',
          amount: c.amount,
          status: c.status
        });
      });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-brand-950/40 border border-brand-800/30 p-4 rounded-xl shadow-glass flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-500 font-semibold uppercase">Total Invoice Sales</p>
            <h3 className="text-lg font-bold text-slate-100 mt-1">
              {currency} {totalInvoices.toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </h3>
          </div>
        ...
        <div className="bg-brand-950/40 border border-brand-800/30 p-4 rounded-xl shadow-glass flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-500 font-semibold uppercase">POS Cash Sales</p>
            <h3 className="text-lg font-bold text-slate-100 mt-1">
              {currency} {totalPOS.toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </h3>
          </div>
        ...
        <div className="bg-brand-950/40 border border-brand-800/30 p-4 rounded-xl shadow-glass flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-500 font-semibold uppercase">Total Cash Receipts</p>
            <h3 className="text-lg font-bold text-slate-100 mt-1">
              {currency} {totalReceipts.toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </h3>
          </div>
        ...
        <div className="bg-brand-950/40 border border-brand-800/30 p-4 rounded-xl shadow-glass flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-500 font-semibold uppercase">Outstanding PDCs</p>
            <h3 className="text-lg font-bold text-slate-100 mt-1">
      ...
      <div className="flex justify-between items-center gap-4">
        ...
        <div className="text-xs text-slate-500 flex items-center gap-1.5 bg-brand-900/15 border border-brand-800/30 px-3 py-1.5 rounded-lg shrink-0">
          ...
          <span>Real-time Unified Sales Ledger Log</span>
        </div>
      </div>
                  ...
                  filteredTransactions.map((tx, idx) => (
                    <tr key={idx} className="hover:bg-brand-900/10 transition-colors">
                      ...
                      <td className={`p-4 text-right font-bold ${
                        tx.type === 'Customer Payment' ? 'text-emerald-400' : 'text-slate-200'
                      }`}>
