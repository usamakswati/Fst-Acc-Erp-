...
interface BillGridProps {
  lines: BillLineItem[];
  setLines: React.Dispatch<React.SetStateAction<BillLineItem[]>>;
  products: any[];
  taxRate: number;
  currency: string;
}
...
export default function BillGrid({
  ...
  const updateLine = (index: number, field: keyof BillLineItem, value: any) => {
    const updated = [...lines];
    ...
    if (field === 'productId') {
      ...
      if (selectedProd) {
        ...
        line.unitCost = selectedProd.costPrice || 0.0;
      } else {
        line.productId = '';
        line.sku = '';
        line.name = '';
      ...
      line.unitCost = Math.max(0, parseFloat(value) || 0);
    ...
    const rawSub = line.quantity * line.unitCost;
  ...
  const calculateTotals = () => {
    ...
    lines.forEach((line) => {
      const rawLineSub = line.quantity * line.unitCost;
          ...
          <table className="w-full text-left border-collapse text-sm min-w-[800px]">
            <thead>
              <tr className="border-b border-brand-800 bg-brand-950/60 text-slate-300">
                ...
                <th className="px-3 py-2.5 font-medium w-24">Qty Received</th>
                <th className="px-3 py-2.5 font-medium w-28">Unit Cost</th>
                <th className="px-3 py-2.5 font-medium w-24">Discount %</th>
                <th className="px-3 py-2.5 font-medium w-32">Tax (GST)</th>
                <th className="px-3 py-2.5 font-medium w-32 text-right">Line Net Cost</th>
